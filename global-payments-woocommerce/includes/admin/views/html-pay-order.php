<?php if ( isset( $render_button ) && $render_button ) : ?>
...
<?php endif; ?>
<script type="text/template" id="tmpl-wc-globalpayments-pay-order-modal-<?php echo esc_attr( $this->id ); ?>">
			...
			<section class="wc-backbone-modal-main" role="main">
				...
				<article>
					<form id="wc-globalpayments-pay-order-form">
						...
						value="<?php echo esc_html( \GlobalPayments\Api\Entities\Enums\ManualEntryMethod::MOTO ); ?>"/>
						<input type="hidden" name="order_id" value="<?php echo esc_html( $order->get_id() ); ?>"/>
						<input type="hidden" name="order_key" value="<?php echo esc_html( $order->get_order_key() ); ?>"/>
						<div class="payment_methods" style="display: none">
							<input type="radio" class="input-radio" name="payment_method"
							value="<?php echo esc_html( $this->id ); ?>" checked="checked">
						</div>
						<?php $this->environment_indicator(); ?>
						<div class="wc_payment_method payment_method_<?php echo esc_html( $this->id ); ?> payment_box">
							...
							<input id="wc-<?php echo esc_html( $this->id ); ?>-payment-token-{{ method.id }}"
							type="radio"
							name="wc-<?php echo esc_html( $this->id ); ?>-payment-token"
							value="{{ method.id }}"
							style="width:auto;"
							class="woocommerce-SavedPaymentMethods-tokenInput"<#if( method.is_default
							){#> checked="checked"<#}#>/>
							<label for="wc-<?php echo esc_html( $this->id ); ?>-payment-token-{{ method.id }}">{{
						...
						<fieldset id="wc-<?php echo esc_attr( $this->id ); ?>-txn-form">
							...
							value="<?php echo esc_html( $order->get_transaction_id() ); ?>"/>
